...
#define NUM_LEDS 84 // how many TOTAL pixels are going to be used here (2 rings 1*60px and 1*12px)
...
void setup() {
  ...
  */
  // rtc.adjust(DateTime(2018, 04, 19, 12, 35, 45));
}
...
void loop() {
  ...
  int hrPix = now.hour() + 60; // pushes the hours out of the min/sec ring, and onto its own strip
...
//    hrPix = hrPix - 12;
//  }
  if (now.hour() != 23) { // removes last hour pixel, if its not 11 anymore
    pixel[60 + 23] = bgColor;
  }
...
void fillTix() {
  ...
  pixel[60] =  tixColor; pixel[63] =  tixColor; pixel[66] =  tixColor; pixel[69] =  tixColor;
    pixel[72] =  tixColor; pixel[75] =  tixColor; pixel[78] =  tixColor; pixel[81] =  tixColor;
}
